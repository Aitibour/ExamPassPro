...
export default async function ExamIndexPage() {
  ...
  if (purchases?.length === 1) {
    redirect(`/exam/${purchases[0].course_id}`)
  }
          ...
          {purchases.map(p => (
            ...
            >
              ...
              <div className="flex items-center justify-between text-xs text-slate-400">
                <span>Mock exam available</span>
                <span className="text-sky-500 font-semibold group-hover:translate-x-1 transition-transform inline-block">Start →</span>
              </div>
            </Link>
          ))}
        </div>
