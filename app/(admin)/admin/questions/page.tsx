...
export default async function AdminQuestionsPage() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-xl font-black text-slate-100">Questions</h1>
        <p className="text-slate-500 text-sm mt-1">Manage exam question bank</p>
      </div>

      {/* Bulk import */}
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 mb-6">
        <h2 className="font-bold text-slate-200 mb-4">Bulk Import</h2>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-800 flex justify-between items-center">
          <span className="font-bold text-slate-300 text-sm">Recent Questions (last 50)</span>
        </div>
          ...
          <tbody className="divide-y divide-slate-800">
            {questions?.map(q => (
              <tr key={q.id} className="hover:bg-slate-800/50 transition-colors">
                <td className="px-5 py-3.5 text-sm text-slate-300 max-w-xs truncate">{q.body}</td>
                ...
                <td className="px-5 py-3.5 text-sm font-mono font-bold text-sky-400">{q.correct}</td>
              </tr>
            ))}
